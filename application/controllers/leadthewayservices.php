			...
			if($user_id == false || $user_id ==""){
				...
				/*if($event_type == 'public'){
					...
					foreach($data1 as $dt){
						...
						$follow = $this->userModel->addFollower($dt,$res);
						//echo 'this is device friend id '.$dt['friend_id'];exit;
						$getDevices = $this->userModel->getDevice($dt['friend_id']);
						//echo '<pre>';print_r($getDevices);exit;
						$device_id = $getDevices[0]['device_id'];
						//echo ($getDevices[0]['device_id']);exit;
						$notes = $this->userModel->getUserNotifications($dt['friend_id']);
						//echo '<pre>';print_r($notes);exit;
						foreach($notes as $n){
						$followerName = $n['UserName'];
						...
						if($device == 'iPhone'){
							...
							$this->userModel->sendPushNotification($device_id,'salsoft',$msg,$badge=0);
						}else{
							//$this->userModel->sendPushNotification($device_id,$passphrase='salsoft',$msg,$badge=0);
							$this->userModel->sendNotification($device_id,$msg);
						}
					//}
						
						/*end push notificatin*/
					}
				}
						$this->_jsonData['status']="SUCCESS";
	...
	public function getEvents(){
		...
		try{
			if($user_id =="" || $user_id == false){
				...
				foreach($data as $dt){
					$data1[$j]=$dt;
					$data1[$j]['LeadLatLongs'] = $this->eventsModel->getEventLeadLatLongs($dt['event_id']);
					$data1[$j]['friends'] = $this->eventsModel->getEventFriendsByEventId($dt['event_id']);
					$j++;
				}
				$this->_jsonData['status']="SUCCESS";
			...
			if($user_id =="" || $user_id == false){
				...
				/*if($routeType == 'public'){
					...
					foreach($data1 as $dt){
						...
						//echo 'this is device friend id '.$dt['friend_id'];exit;
						$getDevices = $this->userModel->getDevice($dt['friend_id']);
						//echo '<pre>';print_r($getDevices);
						$device_id = $getDevices[0]['device_id'];
						//echo 'this is device_id : '.$getDevices[0]['device_id'];
						$notes = $this->userModel->getUserNotifications($dt['friend_id']);
						//echo '<pre>';print_r($notes);exit;
						foreach($notes as $n){
							$followerName = $n['UserName'];
							...
							if($device == 'android'){
								$this->userModel->sendNotification($device_id,$msg);
								//$this->readNotifications($dt['friend_id']);
								$this->userModel->updateNotifications($dt['friend_id']);
							}else{
								$this->userModel->sendPushNotification($device_id,$passphrase='salsoft',$msg,$badge=0);
								//$this->readNotifications($dt['friend_id']);
								$this->userModel->updateNotifications($n['NotificationId']);
							}
			...
			if($user_id == false || $user_id == ""){
				...
				if($status == 'public'){
					...
					if($checkFollower == 0){
						...
						$this->userModel->acceptFriendRoute($user_id,$friend_id,$route_id,$lat,$long);
						$getDevices = $this->userModel->getDevice($friend_id);
						//echo '<pre>';print_r($getDevices);exit;
						$device_id = $getDevices[0]['device_id'];
						$notes = $this->userModel->getUserNotifications($friend_id);
						//echo '<pre>';print_r($notes);exit;
						foreach($notes as $n){
							$followerName = $n['FollowerName'];
							$eventName = $n['event_name'];
							$this->_jsonData['status']="SUCCESS";
							$this->_jsonData['message']="Route Updated Successfully";
							$msg ='The "'.$followerName.'" has Joined your "'.$eventName.'"';
							if($device == 'iPhone'){
								//var_dump($device);exit;
								//$this->userModel->sendNotification($device_id,$msg);
								$this->userModel->sendPushNotification($device_id,'salsoft',$msg,$badge=0);
			...
			if($user_id == false || $user_id == ""){
				...
				$this->_jsonData['message']="user_id Missing";
					...
					foreach($data as $dt){
						...
						$this->userModel->sendNotification($device_id,$msg);
						$this->userModel->sendPushNotification($device_id,$passphrase='salsoft',$msg,$badge=1);
...
';
	...
	try{
		if($user_id == false || $user_id == ""){
			...
			$j=0;
			foreach($data as $dt){
				$this->userModel->updateNotifications($dt['NotificationId']);
				$j++;
			}
			//$this->_jsonData['status']="SUCCESS";
			//$this->_jsonData['message']="Notifications updated Successfully";
		}
		echo json_encode($this->_jsonData);
	}catch(Exception $e){
				$this->_jsonData['status']="FAILURE";
		...
		$this->ServicesModel->createService($_REQUEST,$this->_jsonData,$_SERVER['SERVER_ADDR'],'readNotifications',$_FILES);
		
	}	
	
	public function destination(){
		$user_id = $this->input->get_post('user_id');
		$friend_name = $this->input->get_post('friend_name');
		$device = $this->input->get_post('device');	
		try{
			if($user_id == false || $user_id == ""){
				$this->_jsonData['status']="FAILURE";
				$this->_jsonData['message']="User Id Missing";
			}if($friend_name == false || $friend_name == ""){
				$this->_jsonData['status']="FAILURE";
				$this->_jsonData['message']="friend_name Missing";
			}else{
					$getDevices = $this->userModel->getDevice($user_id);
					$device_id = $getDevices[0]['device_id'];
					$notes = $this->userModel->getUserNotifications($user_id);
					foreach($notes as $n){
						$eventName = $n['event_name'];
						$this->_jsonData['status']="SUCCESS";
						$this->_jsonData['message']="Destination reached";
						$msg ='The "'.$friend_name.'" has Reached the "'.$eventName.'" destination';
						if($device == 'iPhone'){
							$this->userModel->sendPushNotification($device_id,'salsoft',$msg,$badge=0);
							$this->userModel->updateNotifications($n['NotificationId']);
						}else{
		...
		$this->ServicesModel->createService($_REQUEST,$this->_jsonData,$_SERVER['SERVER_ADDR'],'destination',$_FILES);
	} 
	...
	try{
		if($user_id == false || $user_id == ""){
			$this->_jsonData['status']="FAILURE";
			$this->_jsonData['message']="User Id Missing";
		}else if($group_name == false || $group_name == ""){
			$this->_jsonData['status']="FAILURE";
			$this->_jsonData['message']="group_name Missing";
		}else{
			$insert = array(
						'user_id'=>$user_id,
			...
			$group_id = $this->eventsModel->addGroup($insert);
		
			$this->_jsonData['status']="SUCCESS";
	 		$this->_jsonData['message']="Group Added successfully";
		...
		$this->ServicesModel->createService($_REQUEST,$this->_jsonData,$_SERVER['SERVER_ADDR'],'addGroup',$_FILES);
	...
	try{
			if($group_id == false || $group_id == ""){
				$this->_jsonData['status']="FAILURE";
				$this->_jsonData['message']="group_id Missing";
			}else{
					$data = $this->eventsModel->deleteGroup($group_id);
					$this->eventsModel->deleteGroupNumbers($group_id);
					$this->_jsonData['status']="SUCCESS";
					$this->_jsonData['message']="Group Deleted Successfully";
			}
		echo json_encode($this->_jsonData);
	}catch(Exception $e){
				$this->_jsonData['status']="FAILURE";
		...
		$this->ServicesModel->createService($_REQUEST,$this->_jsonData,$_SERVER['SERVER_ADDR'],'deleteGroup',$_FILES);
	...
	try{
			if($user_id == false || $user_id == ""){
				...
				$this->_jsonData['message']="user_id Missing";
					...
					$data = $this->eventsModel->getAllGroups($user_id);
					$j=0;
					foreach($data as $dt){
						$data[$j]['Numbers'] = $this->eventsModel->getAllGroupNumbers($dt['id']);
						$j++;
					}
						$this->_jsonData['status']="SUCCESS";
						$this->_jsonData['message']="Group Viewed Successfully";
						$this->_jsonData['data']=$data; 
			}
		echo json_encode($this->_jsonData);
	}catch(Exception $e){
		...
		$this->ServicesModel->createService($_REQUEST,$this->_jsonData,$_SERVER['SERVER_ADDR'],'getAllGroups',$_FILES);
	...
	public function addGroupNumbers(){
		...
		$data = $this->input->get_post('numbers');
	try{
		if($group_id == false || $group_id == ""){
			$this->_jsonData['status']="FAILURE";
			$this->_jsonData['message']="group_id Missing";
		}else if($contact_name == false || $contact_name == ""){
			$this->_jsonData['status']="FAILURE";
			$this->_jsonData['message']="contact_name Missing";
		}else if($data == false || $data == ""){
			$this->_jsonData['status']="FAILURE";
			$this->_jsonData['message']="Numbers Missing";
		}
		else
		{
			$data1 = explode(",", $data);
			...
			foreach($data1 as $dt){
				...
				$contactName = $name[$i];
				$i++;
				$checkNumbers = $this->eventsModel->checkNumbers($number,$contactName,$group_id);	
				if($checkNumbers['total']==0){
					$groupNumbers = array('group_id'=>$group_id,'name'=>$contactName,'group_number'=>$number);
					$this->eventsModel->addGroupNumbers($groupNumbers);
				}
			}
			$this->_jsonData['status'] = "SUCCESS";	
			$this->_jsonData['message']="Numbers Added successfully";		
			$this->_jsonData['data']=$data;
		}
		...
		$this->ServicesModel->createService($_REQUEST,$this->_jsonData,$_SERVER['SERVER_ADDR'],'addGroupNumbers',$_FILES);
			
	}
	
	public function updateGroupNumbers(){
		$group_id = $this->input->get_post('group_id');
	try{
		if($group_id == false || $group_id == ""){
			$this->_jsonData['status']="FAILURE";
			...
			$numbers = $this->eventsModel->getAllGroupNumbers($group_id);
			if(count($numbers)>0){
				$this->_jsonData['status']="SUCCESS";
				$this->_jsonData['message']="Numbers viewed successfully";
				$this->_jsonData['data']=$numbers;
			}else{
				$this->_jsonData['status']="FAILURE";
				$this->_jsonData['message']="No Numbers found";
			}
		...
		$this->ServicesModel->createService($_REQUEST,$this->_jsonData,$_SERVER['SERVER_ADDR'],'updateGroupNumbers',$_FILES);
	...
	try{
		if($number_id == false || $number_id == ""){
			$this->_jsonData['status']="FAILURE";
			$this->_jsonData['message']="number_id Missing";
		}else{
			$this->eventsModel->deleteGroupNumber($number_id);
			$this->_jsonData['status']="SUCCESS";
	 		$this->_jsonData['message']="Number deleted successfully";
		}
			echo json_encode($this->_jsonData);
	}catch(Exception $e){
				$this->_jsonData['status']="FAILURE";
...
/*	public function logOut(){
		...
		try{
			if($user_id == false || $user_id == ''){
				...
				$signOut = $this->userModel->logOut($user_id);
				$this->_jsonData['status']="SUCCESS";
				$this->_jsonData['message']="user Logged out Successfully";
				//$this->_jsonData['Data']=$signOut; 
			}
			echo json_encode($this->_jsonData);
		}catch(Exception $e){
				$this->_jsonData['status']="FAILURE";
		...
		$this->ServicesModel->createService($_REQUEST,$this->_jsonData,$_SERVER['SERVER_ADDR'],'logOut',$_FILES);
	}	*/
	...
	/*public function sendpush(){
		//echo 'send push';
		$this->userModel->sendPushNotification('7ac3ad8d4660e083706313e4fec9da1fb447e35848b40f97ce4228033c060ca1','salsoft','this is testing push',1);
	}*/
