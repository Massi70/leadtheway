...
class UserModel extends CI_Model {
	...
	public function checkUser($user_email){
		$sql = "select user_id,user_name,user_email,password,CONCAT('".base_url()."uploads/',image) as image,status from users where user_email='".$user_email."'";
		$query=$this->db->query($sql);
		$data=$query->row_array();    // fetches record from the user tables
	  	return 	$data;
   }
   ...
   function insertUserDeviceID($data){	
	...
	function addFollower($data1,$res){
		...
		$sql = "insert into event_friends values(null,'".$data1['user_id']."','".$res."','".$data1['friend_id']."','start',0,0)";
		$this->db->query($sql);
...
/*	function getUsersFriendNotifications($user_id,$friend_id){
	...
	function getDevice($table,$condition){
	...
	function sendPushNotification($device_id,$passphrase,$message,$badge){
