...
class EventsModel extends CI_Model
	...
	public function getLocation($user_id){
		$sql = "select e.user_id,e.latitude,e.longitude,u.user_name from event_location as e, 
					users as u where e.user_id = u.user_id and u.user_id = '".$user_id."'";
	...
	public function getAllEvents($user_id,$phone){
		...
		$sql = "(select e.user_id, e.event_id, e.event_name, e.event_type,u.user_name as leader_name, 
						...
						and e_f.status !='reject' and e.create_type='events' AND e.status = '1'
						group by e.event_id)
						...
						left JOIN leadlatlongs as lll on lll.event_id = e.event_id
						INNER JOIN users AS u ON u.user_id = e.user_id
						where e.event_type='public' and e.create_type='events' AND e.status = '1'
						group by e.event_id)
						order by event_id desc";
		
		
		/*this is the previous working query before tables leadlatlongs added*/
...
and e.create_type='routes' group by e.event_id";*/
	...
	public function getEventLeadLatLongs($event_id){
		...
		$sql = "SELECT ll.leadlat as LeadLat,ll.leadlong as LeadLong
					...
					left join leadlatlongs as ll on ll.event_id = e.event_id 
					and ll.user_id = e.user_id
					where e.event_id = '".$event_id."' and e.create_type = 'events' 
					order by ll.id desc";
		$query = $this->db->query($sql);
		$data = $query->result_array();
		return $data;
   ...
   public function checkEventStatus($route_id){
	...
	public function checkFollower($user_id,$route_id,$friend_id){
		$sql = "select id,user_id,friend_id,event_id,latitude,longitude from event_friends
				 where user_id = '".$friend_id."' and event_id = '".$route_id."' and friend_id = '".$user_id."' "; 
		$query = $this->db->query($sql);
		$res = $query->num_rows();
		return $res;
    }
	...
	public function addGroupNumbers($groupNumbers){
		$result = $this->db->insert('group_numbers',$groupNumbers);
		return $this->db->insert_id();
	}
	
	public function addGroup($insert){
		$result = $this->db->insert('groups',$insert);
		return $this->db->insert_id();
	}
	
	public function deleteGroupNumbers($group_id){
		$sql = "delete from group_numbers where group_id = '".$group_id."' "; 
		$this->db->query($sql);
    }
	...
	public function getAllGroups($user_id){
		$sql =	"SELECT id,user_id,group_name,STATUS,date_time 
				FROM groups 
	...
	public function getAllGroupNumbers($group_id){
		$sql =	"SELECT id,group_id,group_number,name as contact_name 
				FROM group_numbers
	...
	public function deleteGroupNumber($number_id){
		$sql =	"delete	FROM group_numbers WHERE id = '".$number_id."'";
		$query = $this->db->query($sql);	
	}
	
	public function checkNumbers($data,$contact_name, $group_id){
	$sql =	"select count(id) as total FROM group_numbers WHERE group_number='".$data."' and name='".$contact_name."' and group_id =".$group_id;
		$query = $this->db->query($sql);
